...
import { getTextContent, hasSlot } from './internal/slot';
...
export default class SlSelect extends LitElement {
  ...
  async handleSlotChange() {
    ...
    items.map((item) => {
      if (values.includes(item.value)) {
        console.error(
          `Duplicate value found in <sl-select> menu item: '${item.value}'`,
          item
        );
      }
  ...
  syncItemsFromValue() {
    ...
    if (this.multiple) {
      ...
      this.displayLabel = checkedItems[0]
        ? this.getItemLabel(checkedItems[0])
        : '';
      ...
      if (
        ...
        this.displayTags.push(html`
          <sl-tag exportparts="base:tag" type="neutral" size=${this.size}> +${
  ...
  syncValueFromItems() {
    ...
    if (this.multiple) {
      this.value = (this.value as []).filter((val) =>
  ...
  render() {
    const hasSelection = this.multiple
      ? this.value?.length > 0
      : this.value !== '';
    ...
    return renderFormControl(
      ...
      html`
        ...
        >
          ...
          <sl-menu part="menu" class="select__menu" @sl-select=${
