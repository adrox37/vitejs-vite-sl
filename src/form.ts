...
export default class SlForm extends LitElement {
  ...
  connectedCallback() {
    ...
    this.formControls = [
          ...
          if (el.name && !el.disabled) {
            if (el.multiple) {
              ...
              if (selectedOptions.length) {
                selectedOptions.map((option: HTMLOptionElement) =>
      ...
      {
        tag: 'sl-button',
        serialize: (el: SlButton, formData) =>
          el.name && !el.disabled ? formData.append(el.name, el.value) : null,
        click: (event) => {
      ...
      },
      {
        ...
        serialize: (el: SlCheckbox, formData) =>
          el.name && el.checked && !el.disabled
            ? formData.append(el.name, el.value)
            : null,
      },
      {
      ...
      {
        tag: 'sl-textarea',
        serialize: (el: SlTextarea, formData) =>
          el.name && !el.disabled ? formData.append(el.name, el.value) : null,
      },
      {
        tag: 'textarea',
